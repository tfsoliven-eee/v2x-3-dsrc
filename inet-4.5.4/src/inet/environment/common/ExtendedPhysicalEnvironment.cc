//
// ExtendedPhysicalEnvironment.cc
//

#include "inet/environment/common/ExtendedPhysicalEnvironment.h"
...
Define_Module(ExtendedPhysicalEnvironment);

ExtendedPhysicalEnvironment::~ExtendedPhysicalEnvironment()
{
    //for (auto &elem : movingVehicles)
    //    delete elem.second;
...
void ExtendedPhysicalEnvironment::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        EV_INFO << "ExtendedPhysicalEnvironment::initialize INITSTAGE_LOCAL" << endl;
        ...
        cModule *managerModule = getModuleByPath("manager");
        if (!managerModule)
            managerModule = getSimulation()->findModuleByPath("**.manager");

        if (managerModule) {
            managerModule->subscribe("org_car2x_veins_modules_mobility_traciModuleAdded", this);
            managerModule->subscribe("org_car2x_veins_modules_mobility_traciModuleRemoved", this);
            EV_INFO << "ExtendedPhysicalEnvironment::initialize Subscribing to manager at " << managerModule->getFullPath() << endl;

            EV_INFO << "Subscribed to traciModuleAdded and traciModuleRemoved from "
                    << managerModule->getFullPath() << endl;
        }
    }
        ...
        EV_INFO << "ExtendedPhysicalEnvironment::initialize INITSTAGE_PHYSICAL_ENVIRONMENT" << endl;

        //if (getSimulation()->getSystemModule()->hasSubmodule("visualizer")) {
        //    emit(physicalObjectAddedSignal, object); // safe emit
        //}
    } else if (stage == INITSTAGE_LAST) {
        EV_INFO << "ExtendedPhysicalEnvironment::initialize INITSTAGE_LAST stage " << stage << endl;
        //registerAllVehicles();
...
void ExtendedPhysicalEnvironment::receiveSignal(cComponent *src, simsignal_t signalID, cObject *obj, cObject *details)
{
    const char *signalName = getSignalName(signalID);
    EV_INFO << "[ExtendedPhysicalEnvironment::receiveSignal] got signal " << (signalName ? signalName : "(unknown)") 
            << " from " << (src ? src->getFullPath() : "null") << endl;

    // detect traciModuleAdded by name
    if (signalName && strstr(signalName, "traciModuleAdded")) {
        cModule *mod = dynamic_cast<cModule *>(obj);
        if (mod) {
            if (mod->getFullPath().find("car") != std::string::npos) {
                EV_INFO << "[ExtendedPhysicalEnvironment::receiveSignal] traciModuleAdded -> " << mod->getFullPath() << endl;
                registerVehicle(mod);
            }
        }
        else {
            EV_WARN << "ExtendedPhysicalEnvironment::receiveSignal traciModuleAdded: obj is not a module" << endl;
        }
    }
    else if (signalName && strstr(signalName, "traciModuleRemoved")) {
        // handle removal if you want
    }
    else if (strstr(signalName, "mobilityStateChanged")) {
        // mobility updates — src is mobility module
        IMobility *mob = check_and_cast<IMobility *>(src);
        Coord pos = mob->getCurrentPosition();
        cModule *vehicle = src->getParentModule(); // or src->getParentModule()
        EV_INFO << "[ExtendedPhysicalEnvironment::receiveSignal] receiveSignal vehicle value " << vehicle << endl;
        if (vehicle) {
            EV_INFO << "[ExtendedPhysicalEnvironment::receiveSignal] receiveSignal -> updateVehiclePosition" << endl;
            updateVehiclePosition(vehicle->getId(), pos, Coord(2,1.5,1.5));
        }
    }
}


void ExtendedPhysicalEnvironment::registerVehicle(cModule *vehModule)
{
    cModule *mobilityModule = vehModule->getSubmodule("mobility");
    if (!mobilityModule) {
        EV_WARN << "ExtendedPhysicalEnvironment::registerVehicle" << vehModule->getFullPath() << " has no mobility submodule!" << endl;
        return;
    }

    // subscribe to mobility changes
    mobilityModule->subscribe(IMobility::mobilityStateChangedSignal, this);

    EV_INFO << "ExtendedPhysicalEnvironment::registerVehicle" << mobilityModule << endl;
    IMobility *mob = check_and_cast<IMobility *>(mobilityModule);
    Coord pos = mob->getCurrentPosition();
    Coord size(4, 2, 1.5);

    EV_INFO << "ExtendedPhysicalEnvironment::registerVehicle Registering moving vehicle " << vehModule->getFullPath() << " at " << pos.str() << endl;
    addMovingVehicle(vehModule->getId(), pos, size);
}


// Vehicle handling
void ExtendedPhysicalEnvironment::addMovingVehicle(int id, const Coord& position, const Coord& size)
{
    EV_INFO << "ExtendedPhysicalEnvironment::addMovingVehicle1" << endl;
    //if (movingVehicles.find(id) != movingVehicles.end()) return;

    if (idToObjectMap.find(id) != idToObjectMap.end())
        return;

    Cuboid *shape = new Cuboid(size);
    const Material *m = MaterialRegistry::getInstance().getMaterial("aluminum");
    Material *aluminum = const_cast<Material *>(m);

    PhysicalObject *vehicleObject = new PhysicalObject("vehicle", id, position, Quaternion(), shape, aluminum, 1.0, cFigure::BLACK, cFigure::WHITE, 1.0, nullptr, nullptr);  
    objects.push_back(vehicleObject);  
    //movingVehicles[id] = vehicleObject;
    if (id != -1)
        idToObjectMap.insert(std::pair<int, const PhysicalObject *>(id, vehicleObject));
    EV_INFO << "ExtendedPhysicalEnvironment::addMovingVehicle Vehicle object added (id=" << id << ") at pos = " << position << endl;
}


void ExtendedPhysicalEnvironment::updateVehiclePosition(int id, const Coord& position, const Coord& size)
{
    EV_INFO << "[ExtendedPhysicalEnvironment::updateVehiclePosition] Objects Before deletion: size = " << objects.size() 
    << " | last object = " << objects.back() << " | last object ID = " << objects.back()->getId() << " | last object pos = " << objects.back()-> getPosition() << endl;
    
    // Find the existing object by id in objects vector
    auto it = std::find_if(objects.begin(), objects.end(), [id](const PhysicalObject *obj) { return obj->getId() == id; });
    ...
    EV_INFO << "[ExtendedPhysicalEnvironment::updateVehiclePosition] Objects After deletion: size = " << objects.size() 
    << " | last object = " << objects.back() << " | last object ID = " << objects.back()->getId() << " | last object pos = " << objects.back()-> getPosition() << endl;

    // Recreate with new position
    addMovingVehicle(id, position, size);
}



void ExtendedPhysicalEnvironment::convertPoints(std::vector<Coord>& points)
{
...
void ExtendedPhysicalEnvironment::parseShapes(cXMLElement *xml)
{
...
void ExtendedPhysicalEnvironment::parseMaterials(cXMLElement *xml)
{
...
void ExtendedPhysicalEnvironment::parseObjects(cXMLElement *xml)
{
...
// Object access
const PhysicalObject *ExtendedPhysicalEnvironment::getObject(int index) const
{
    if (index < (int)objects.size())
        return objects[index];
    index -= objects.size();
    //if (index < (int)movingVehicles.size()) {
    //    auto it = movingVehicles.begin();
...
const PhysicalObject *ExtendedPhysicalEnvironment::getObjectById(int id) const
{
    auto it = idToObjectMap.find(id);
    return (it != idToObjectMap.end()) ? it->second : nullptr;
}

void ExtendedPhysicalEnvironment::visitObjects(const IVisitor *visitor, const LineSegment& lineSegment) const
{
    if (objectCache)
