...
// ===================================================================================================
// for PEDESTRIANS
std::list<std::string> TraCICommandInterface::getPersonIds() {
...
std::vector<std::tuple<std::string, int, double, char>> TraCICommandInterface::Vehicle::getNextTls()
{
    ...
    for (unsigned int i = 0; i < numLinks; ++i) {
        ...
        result.push_back(std::tuple(tlsId, tlsLinkIndex, distanceToTls, linkState)); // make_tuple -> tuple
    }
...
std::tuple<std::string, double, uint8_t> TraCICommandInterface::getRoadMapPos(const Coord& coord)
{
    ...
    return std::tuple(convRoadId, convPos, convLaneId);// make_tuple -> tuple
}
