...
void TraCIScenarioManager::init_traci()
{
    ...
    // for PEDESTRIANS
    {
        // subscribe to list of pedestrian ids
        simtime_t beginTime = 0;
        simtime_t endTime = SimTime::getMaxTime();
        std::string objectId = "";
        uint8_t variableNumber = 1;
        uint8_t variable1 = ID_LIST;
        TraCIBuffer bufPed = connection->query(
            CMD_SUBSCRIBE_PERSON_VARIABLE,
            TraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1);

        processSubcriptionResult(bufPed);
        ASSERT(bufPed.eof());
    }
    // end for pedestrians
    // ===================================================================================================
...
void TraCIScenarioManager::addModule(std::string nodeId, std::string type, std::string name, std::string displayString, const Coord& position, std::string road_id, double speed, Heading heading, VehicleSignalSet signals, double length, double height, double width)
{
    ...
    EV_INFO << "TraCIScenarioManager::addModule" << endl;
}
...
// ===================================================================================================
// for PEDESTRIANS
void TraCIScenarioManager::subscribeToPedestrianVariables(std::string personId)
{
    EV_INFO << "[TraCIScenarioManager::subscribeToPedestrianVariables]" << endl;
    simtime_t beginTime = 0;
    simtime_t endTime = SimTime::getMaxTime();
    ...
    TraCIBuffer buf1;
    buf1 << beginTime << endTime << objectId << variableNumber;
    for (auto variable : variables) buf1 << variable;
...
void TraCIScenarioManager::unsubscribeFromPedestrianVariables(std::string personId)
{
    EV_INFO << "[TraCIScenarioManager::unsubscribeFromPedestrianVariables]" << endl;
    simtime_t beginTime = 0;
    simtime_t endTime = SimTime::getMaxTime();
    std::string objectId = personId;
    uint8_t variableNumber = 0;

    TraCIBuffer buf = connection->query(CMD_SUBSCRIBE_PERSON_VARIABLE,
...
void TraCIScenarioManager::processSimSubscription(std::string objectId, TraCIBuffer& buf)
{
    ...
    // ===================================================================================================
    // for PEDESTRIANS
    // Detect new pedestrians
    EV_INFO << "[TraCIScenarioManager::processSimSubscription]" << endl;

    // Subscribe to pedestrians
        auto personIds = getCommandInterface()->getPersonIds();
        for (const auto& pid : personIds) {
            auto inserted = subscribedPedestriansList1.insert(pid);
            if (inserted.second) {
                EV_INFO << "Subscribing to new pedestrian " << pid << endl;
                subscribeToPedestrianVariables(pid);
            }
        }

        // Detect pedestrians that have left SUMO
        std::set<std::string> currentPedestrianSet(personIds.begin(), personIds.end());
        for (auto it = subscribedPedestriansList1.begin(); it != subscribedPedestriansList1.end(); ) {
            if (currentPedestrianSet.find(*it) == currentPedestrianSet.end()) {
                EV_INFO << "Pedestrian " << *it << " has left SUMO. Unsubscribing." << endl;
                unsubscribeFromPedestrianVariables(*it);
                it = subscribedPedestriansList1.erase(it);
            } else {
                ++it;
            }
        }
...
// ===================================================================================================
// for PEDESTRIANS
void TraCIScenarioManager::processPedestrianSubscription(std::string objectId, TraCIBuffer& buf)
{
    EV_INFO << "[TraCIScenarioManager::processPedestrianSubscription]" << endl;
    double px, py, speed, angle_traci;
    std::string edge;
    int numRead = 0;
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j) {
        ...
        if (isokay != RTYPE_OK) {
            ...
            if (varType == TYPE_STRING) {
                std::string errormsg;
                buf >> errormsg;
                EV_WARN << "[TraCIScenarioManager::processPedestrianSubscription] Pedestrian subscription error: " << errormsg << endl;
            }
            continue;
        }

        if (variable1_resp == VAR_POSITION) {
            uint8_t varType; buf >> varType; ASSERT(varType == POSITION_2D);
            buf >> px; buf >> py;
            numRead++;
        }
        else if (variable1_resp == VAR_ROAD_ID) {
            uint8_t varType; buf >> varType; ASSERT(varType == TYPE_STRING);
            buf >> edge;
            numRead++;
        }
        else if (variable1_resp == VAR_SPEED) {
            uint8_t varType; buf >> varType; ASSERT(varType == TYPE_DOUBLE);
            buf >> speed;
            numRead++;
        }
        else if (variable1_resp == VAR_ANGLE) {
    ...
    Coord p = connection->traci2omnet(TraCICoord(px, py));
    double angle = angle_traci;

    Heading heading = connection->traci2omnetHeading(angle_traci);

    EV_INFO << "[TraCIScenarioManager::processPedestrianSubscription] Pedestrian " << objectId
        << " pos=(" << px << "," << py << ")"
        << " omnet=(" << p.x << "," << p.y << ")"
        << " speed=" << speed
        << " angle=" << angle
    ...
    // check ROI
    bool inRoi = !roi.hasConstraints() ? true : (roi.onAnyRectangle(TraCICoord(px, py)) || roi.partOfRoads(edge));
    cModule* mod = pedestrians[objectId];
    if (!inRoi) {
        if (mod) {
            mod->callFinish();
            mod->deleteModule();
            pedestrians.erase(objectId);
            EV_DEBUG << "[TraCIScenarioManager::processPedestrianSubscription] Pedestrian " << objectId << " left ROI" << endl;
        }
        return;
    }

    if (!mod) {
        // Prevent duplicate creation if it already exists in OMNeT++
        if (getManagedModule(objectId)) {
            EV_WARN << "Pedestrian " << objectId << " already exists, skipping addModule() and updating position." << endl;
            EV_DEBUG << "Pedestrian " << objectId << " moving to " << p.x << "," << p.y << endl;
            cModule* modNew = getManagedModule(objectId);
            updateModulePosition(modNew, p, edge, speed, heading, VehicleSignalSet(signals));
            emit(traciModuleUpdatedSignal, modNew);
            return;
        }

        // no such module - need to create
        std::string pType = commandIfc->person(objectId).getTypeId();
        std::string mType, mName, mDisplayString;
        TypeMapping::iterator iType, iName, iDisplayString;

        TypeMapping::iterator i;
        iType = moduleType.find(pType);
        if (iType == moduleType.end()) {
            iType = moduleType.find("*");
            if (iType == moduleType.end()) throw cRuntimeError("cannot find a module type for pedestrian type \"%s\"", pType.c_str());
        }
        mType = iType->second;
        // search for module name
        iName = moduleName.find(pType);
        if (iName == moduleName.end()) {
            iName = moduleName.find(std::string("*"));
            if (iName == moduleName.end()) throw cRuntimeError("cannot find a module name for pedestrian type \"%s\"", pType.c_str());
        }
        mName = iName->second;
        if (moduleDisplayString.size() != 0) {
            iDisplayString = moduleDisplayString.find(pType);
            if (iDisplayString == moduleDisplayString.end()) {
                iDisplayString = moduleDisplayString.find("*");
                if (iDisplayString == moduleDisplayString.end()) throw cRuntimeError("cannot find a module display string for pedestrian type \"%s\"", pType.c_str());
            }
            mDisplayString = iDisplayString->second;
        }
        else {
        ...
        if (mType != "0") {
            ...
            EV_DEBUG << "Added pedestrian #" << objectId << endl;
...
void TraCIScenarioManager::processSubcriptionResult(TraCIBuffer& buf)
{
    ...
    else if (commandId_resp == RESPONSE_SUBSCRIBE_PERSON_VARIABLE) {      // ======================= for PEDESTRIANS
        processPedestrianSubscription(objectId_resp, buf);
    }
    else {
